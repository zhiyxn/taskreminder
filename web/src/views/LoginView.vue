...
const [registerPassword, registerPasswordAttrs] =
  defineRegisterField('registerPassword')
...
const doRegister = handleRegisterSubmit(async (values) => {
  ...
  const result = await auth.register(
    values.registerEmail.trim(),
    values.registerPassword,
    captchaId.value,
    values.captchaCode.trim(),
  )
...
<template>
  ...
  >
    <!-- 背景装饰光斑 -->
    <div
      class="pointer-events-none absolute -top-40 -right-40 size-[28rem] rounded-full bg-primary/15 blur-3xl"
    />
    <div
      class="pointer-events-none absolute -bottom-48 -left-20 size-[22rem] rounded-full bg-primary/10 blur-3xl"
    />
    <div
      class="pointer-events-none absolute top-1/4 left-[10%] size-48 rounded-full bg-primary/10 blur-2xl"
    />
    <div
      class="pointer-events-none absolute top-[60%] right-[5%] size-56 rounded-full bg-primary/10 blur-3xl"
    />
    <div
      class="pointer-events-none absolute top-[15%] right-[20%] size-32 rounded-full bg-primary/20 blur-2xl"
    />
    <div
      class="pointer-events-none absolute bottom-[25%] left-[40%] size-40 rounded-full bg-primary/10 blur-2xl"
    />
    <div
      class="pointer-events-none absolute top-[40%] right-[30%] size-28 rounded-full bg-primary/15 blur-xl"
    />
    <div
      class="pointer-events-none absolute top-[5%] left-[30%] size-36 rounded-full bg-primary/10 blur-2xl"
    />
    <div
      class="pointer-events-none absolute bottom-[10%] right-[25%] size-44 rounded-full bg-primary/10 blur-3xl"
    />
    <div
      class="pointer-events-none absolute top-[70%] left-[5%] size-24 rounded-full bg-primary/10 blur-xl"
    />
    <div
      class="pointer-events-none absolute top-[30%] left-[55%] size-20 rounded-full bg-primary/15 blur-xl"
    />

    <!-- 登录卡片 -->
    <div
      v-if="!showRegister"
      class="relative z-10 w-full max-w-md rounded-lg border border-border bg-card/60 p-8 shadow-2xl backdrop-blur-2xl backdrop-saturate-150"
    >
      <div class="mb-6 text-center">
        <div
          class="mx-auto mb-4 flex size-14 items-center justify-center rounded-2xl bg-primary shadow-lg shadow-primary/25"
        >
      ...
      <form @submit.prevent="doLogin" class="space-y-4">
        <div>
          ...
          />
          <p
            v-if="loginErrors.loginUsername"
            id="login-username-error"
            class="mt-1 text-xs text-destructive"
          >
            {{ loginErrors.loginUsername }}
          </p>
        </div>
        <div>
          <label
          ...
          />
          <p
            v-if="loginErrors.loginPassword"
        ...
        <Button type="submit" :disabled="loginLoading" class="w-full">
    ...
    >
      ...
      <form @submit.prevent="doRegister" class="space-y-4">
        <div>
          ...
          />
          <p
            v-if="registerErrors.registerEmail"
            id="reg-email-error"
            class="mt-1 text-xs text-destructive"
          >
        ...
        <div>
          ...
          />
          <p
            v-if="registerErrors.registerPassword"
        ...
        <div>
          ...
          <div class="flex gap-3">
            ...
            />
            <div
              class="flex h-10 w-32 cursor-pointer items-center justify-center overflow-hidden rounded-md border border-border bg-muted/50"
          ...
          <p
            v-if="registerErrors.captchaCode"
            id="captcha-error"
            class="mt-1 text-xs text-destructive"
          >
        ...
        <Button type="submit" :disabled="registerLoading" class="w-full">
